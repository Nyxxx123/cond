...
def plot_loss_curves(train_losses, mse_losses, lpips_losses, ms_ssim_losses, gan_losses, disc_losses, save_dir="./"):
    """绘制损失曲线"""
    ...
    if ms_ssim_losses:
        axes[0, 1].plot(epochs, ms_ssim_losses, 'orange', label='MS-SSIM', linewidth=2)
    axes[0, 1].set_xlabel('Epoch')
    axes[0, 1].set_ylabel('Loss')
    axes[0, 1].set_title('MSE & LPIPS & MS-SSIM Loss')
    axes[0, 1].legend()
...
def train_one_epoch(model, diffusion, dataloader, optimizer_G, config, epoch, lpips_loss_fn,
    ...
    for batch_idx, batch in enumerate(pbar):
        ...
        if config.use_ms_ssim and ms_ssim is not None:
            ...
            targets_rgb = torch.clamp(targets_rgb, 0, 1)
            # 复制为3通道
            x0_pred_rgb = x0_pred_rgb.repeat(1, 3, 1, 1)
            targets_rgb = targets_rgb.repeat(1, 3, 1, 1)
            ms_ssim_val = ms_ssim(x0_pred_rgb, targets_rgb)  # 相似度 [0,1]
            ms_ssim_loss_val = 1.0 - ms_ssim_val
            total_ms_ssim_loss += ms_ssim_loss_val.item()
        else:
        ...
        if config.use_ms_ssim:
            postfix['ms_ssim'] = f'{ms_ssim_loss_val.item():.4f}'
        if use_gan:
...
def main():
    ...
    print(f"MS-SSIM: {config.use_ms_ssim}, weight={config.ms_ssim_loss_weight}")
    ...
    if config.use_ms_ssim:
        ms_ssim = MultiScaleStructuralSimilarityIndexMeasure(
            kernel_size=config.ms_ssim_kernel_size,
            sigma=config.ms_ssim_sigma,
            reduction='elementwise_mean',
            data_range=1.0
        ).to(config.device)
        print(f"MS-SSIM initialized with weight={config.ms_ssim_loss_weight}")
    # =======================================
    ...
    for epoch in range(config.num_epochs):
        # 训练一个epoch
        avg_loss_G, avg_mse, avg_lpips, avg_ms_ssim, avg_gan, avg_loss_D = train_one_epoch(
            model, diffusion, train_dataloader, optimizer_G, config, epoch,
            lpips_loss_fn, ms_ssim, discriminator, optimizer_D
        )
        scheduler.step()
        ...
        if config.use_ms_ssim:
            ms_ssim_losses.append(avg_ms_ssim)
        if config.use_gan:
        ...
        print(f"    MSE: {avg_mse:.6f}, LPIPS: {avg_lpips:.6f}", end='')
        if config.use_ms_ssim:
            print(f", MS-SSIM: {avg_ms_ssim:.6f}")
        else:
            print()
        if config.use_gan:
        ...
        if (epoch + 1) % config.sample_frequency == 0:
            ...
            # 更新损失曲线
            plot_loss_curves(train_losses, mse_losses, lpips_losses, ms_ssim_losses, gan_losses, disc_losses,
                            save_dir=config.sample_dir)
            save_loss_history({
                'train_losses': train_losses,
                'mse_losses': mse_losses,
                'lpips_losses': lpips_losses,
                'ms_ssim_losses': ms_ssim_losses,

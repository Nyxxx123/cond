...
if __name__ == "__main__":
    ...
    if os.path.exists(config.data_dir):
        ...
        if len(dataset) > 0:
            ...
            print(f"Mask类型: {config.mask_type}")
            ...
            if config.mask_type == "2d":
                ...
                axes[1].imshow(mask_disp, cmap='hot')
                axes[1].set_title("2D MIP掩码")
            else:
                mask_3d = sample['mask'][0].numpy()
                mid_slice = mask_3d.shape[0] // 2
                axes[1].imshow(mask_3d[mid_slice], cmap='hot')
                axes[1].set_title(f"3D掩码 (中间切片 D={mid_slice})")
            axes[1].axis('off')

            plt.tight_layout()
            plt.show()
        else:
            print("没有找到数据，请检查目录结构")
            print(f"期望: {config.data_dir}/angiographs/patient/patient_X_mask.png")
            if config.mask_type == "2d":
                print(f"      {config.data_dir}/mask2D/patient_mip.npy")
            else:
                print(f"      {config.data_dir}/mask/patient.nii.gz")
    else:
        print(f"目录不存在: {config.data_dir}")